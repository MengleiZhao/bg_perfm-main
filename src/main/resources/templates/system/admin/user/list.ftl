            ...
            <div class="box-header">
                ...
                <@shiro.hasPermission name="user/add">
                    <a onclick="securityToListAjax();" class="btn btn-sm btn-primary" target="modal" modal="lg"
                       href="${request.contextPath}/user/add">添加</a>
                </@shiro.hasPermission>
                </div>
            </div>
            <div class="box-body">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        security_tab = $('#security_tab').DataTable({
            ...
            "ajax": {
                "url": "${request.contextPath}/user/page",
                "type": "post",
                "data": function(d) {
                    //自定义查询参数
                    d.roleId = $("#roleId").val();
            ...
            "columnDefs": [
...

                {
                    ...
                    "render": function (data) {
                        var btn = "";
                        btn = '<a class="btn btn-xs btn-primary" target="modal" modal="lg" href="${request.contextPath}/user/view/' + data.id + '">查看</a> &nbsp;';
                        if (isNull(data.role) || 'super' != data.role.value) {
                            btn += '<@shiro.hasPermission name="user/edit">'
                                    + '<a class="btn btn-xs btn-info" onclick="securityToListAjax();" data-title="修改" target="modal" modal="lg" href="${request.contextPath}/user/edit/'+ data.id+ '">修改</a> &nbsp;'
                                    +'</@shiro.hasPermission>'
                                    + '<@shiro.hasPermission name="user/edit">'
                                    + '<a class="btn btn-xs btn-info" onclick="securityToListAjax();" target="modal" modal="lg" href="${request.contextPath}/user/goResetPwd/'+ data.id+ '">重置密码</a> &nbsp;'
                                    +'</@shiro.hasPermission>'
                                    + '<@shiro.hasPermission name="user/edit">'
                                    + '<a class="btn btn-xs btn-info" onclick="securityToListAjax();" target="modal" modal="lg" href="${request.contextPath}/user/goDispatcherRole/'+ data.id+ '">角色分配</a> &nbsp;'
                                    +'</@shiro.hasPermission>'
                                    + '<@shiro.hasPermission name="user/delete">'
                                    + '<a class="btn btn-xs btn-default" callback="securityReload();" data-body="确认要删除吗？" target="ajaxTodo" href="${request.contextPath}/user/delete/'+ data.id + '">删除</a>'
                                    +'</@shiro.hasPermission>';
                        }
                        return btn;
                    }
